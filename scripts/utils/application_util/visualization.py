...
class Visualization(object):
    ...
    def __init__(self, seq_info, loc, thread=None):
        image_shape = seq_info["image_size"]
        # aspect_ratio = float(image_shape[1]) / image_shape[0]
        # image_shape = 1024, 1024
        # image_shape = 1024, int(aspect_ratio * 1024)
        self.viewer = ImageViewer(seq_info["update_ms"], image_shape, "Figure %s" % seq_info["sequence_name"])
    ...
    def update_seq_info(self, seq_info):
        self.last_idx = seq_info["max_frame_idx"]
    ...
    def draw_trackers(self, track_dic, idx,tracks, thickness=None, grey_img=None):
        stride = 3
        ...
        for track in tracks:
            ...
            if thickness:
                ...
                color = grey_img[int(box[1]) + int(box[3]/2)][int(box[0]) + int(box[2]/2)]
                self.viewer.color = (color, color, color)
            else:
                self.viewer.color = create_unique_color_uchar(track.track_id)
            # print(self.viewer.color)
            # self.viewer.rectangle(idx, self.vid_name, *track.to_tlwh().astype(np.int), thread=self.thread, thickness=thickness)
            self.viewer.rectangle(idx, self.vid_name, *track.to_tlwh().astype(np.int), label=str(track.track_id), thread=self.thread, thickness=thickness)
            boxes.append([track.to_tlwh().astype(np.int)])
